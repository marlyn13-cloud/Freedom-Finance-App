...
=======
const TX_KEY  = "ff_transactions_v1";
>>>>>>> 9b6d93a48d35780e07ad8a8aedfbce3bf2d6c821
...
document.addEventListener("keydown", (e) => {
  if (e.key === "Escape") {
    closeModal("txModal"); closeModal("budgetModal");
  }
});
...
function requireAuth() { 
  if (!document.getElementById("authPage") && !getCurrentUser()) window.location.href = "index.html"; 
}
...
function loadBudgets() { 
  try { 
    ...
    if (Array.isArray(b)) return b; } catch { /**/ } const seed = []; saveBudgets(seed); return seed;
   }
...
function loadCategories() { 
  try { 
    ...
    if (Array.isArray(c) && c.length) return c; } catch { /**/ } const defaults = ["Other", "Salary"]; saveCategories(defaults); return defaults;
   }
...
<<<<<<< HEAD
  return { 
    income, expenses, savings, investment, balance: income - expenses 
  };
}
...
<<<<<<< HEAD
    if (idx >= 0) txs[idx] = { 
      id: editingTxId, type, date, desc, amount: storedAmount, category 
    };
  } else { 
...
function clearBudError() { 
  const b = document.getElementById("budError"); if (b) b.style.display = "none"; 
}
=======
